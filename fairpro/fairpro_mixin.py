...
class FairProMixin:
    ...
    def check_prompt_for_bias(
        ...
        Args:
            ...
            temperature: Low temperature for more deterministic output.
        ...
        if response.startswith("no"):
            return False
        return True  # "yes" or ambiguous -> apply FairPro
    ...
    def generate_fairpro_system_prompts_batch(
        ...
        for user_prompt in user_prompts:
            # First, check if this prompt could lead to social bias
            if not skip_bias_check:
                could_lead_to_bias = self.check_prompt_for_bias(user_prompt)
                if not could_lead_to_bias:
                    # No bias concern - use default system prompt
                    logger.info(
                        f"Prompt '{user_prompt[:50]}...' -> No bias concern, using default system prompt"
                    )
                    system_prompts.append(default_system_prompt)
                    continue

            # Bias detected or skip_bias_check=True - generate FairPro system prompt
            logger.info(
                f"Prompt '{user_prompt[:50]}...' -> Potential bias detected, generating FairPro system prompt"
            )
